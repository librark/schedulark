...
describe('SqlQueue', () => {
  ...
  it('initializes the schema on first access', async () => {
    ...
    const indexClause = 'CREATE INDEX IF NOT EXISTS ON'
    expect(dedent(statement)).toEqual(
      dedent(`CREATE TABLE IF NOT EXISTS ${schema} (\n` +
        '"id" UUID PRIMARY KEY,\n' +
        '"data" JSONB);\n' +
        `${indexClause} ${schema} ((data->>'job'));\n` +
        `${indexClause} ${schema} ((data->>'lane'));\n` +
        `${indexClause} ${schema} ((data->>'scheduledAt'));\n` +
        `${indexClause} ${schema} ((data->>'pickedAt'));\n` +
        `${indexClause} ${schema} ((data->>'deletedAt'));\n`
      ).trim())
    expect(parameter).toEqual(undefined)
  })
})
