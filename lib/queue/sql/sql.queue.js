...
export class SqlQueue {
  ...
  async _init () {
    ...
    "id" UUID PRIMARY KEY,
    "data" JSONB);
    CREATE INDEX IF NOT EXISTS ON ${target} ((data->>'job'));
    CREATE INDEX IF NOT EXISTS ON ${target} ((data->>'lane'));
    CREATE INDEX IF NOT EXISTS ON ${target} ((data->>'scheduledAt'));
    CREATE INDEX IF NOT EXISTS ON ${target} ((data->>'pickedAt'));
    CREATE INDEX IF NOT EXISTS ON ${target} ((data->>'deletedAt'));
    `.trim()
