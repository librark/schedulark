...
describe('Worker', () => {
  ...
  it('can retry tasks with backoff', async () => {
    ...
    expect(content.T002.failedAt.getTime()).toEqual(0)
  ...
  it('runs until the max retries are reached', async () => {
    worker.registry.BetaJob = new FailingBetaJob()
    worker.iterations = -7
    worker.sleep = 10
    worker.rest = 1
    ...
    expect(Object.keys(content).length).toEqual(2)
    expect('T002' in content).toBeFalsy()
  })
