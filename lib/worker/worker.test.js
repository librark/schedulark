...
describe('Worker', () => {
  ...
  it('can be passed configuration parameters', () => {
    const worker = new Worker({ rest: 2_000, sleep: 8_000, clean: 3_600_000 })
    expect(worker.rest).toEqual(2_000)
    expect(worker.sleep).toEqual(8_000)
    expect(worker.clean).toEqual(3_600_000)
  })
  ...
  it('runs until all the tasks are processed', async () => {
    ...
    expect(worker.iterations).toEqual(0)
    expect(worker.registry.BetaJob.runTasks.length).toEqual(6)
    expect(Object.keys(content).length).toEqual(0)
  })
})
